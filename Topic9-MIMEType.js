...
async function handleStaticFile(socket, path) {
    ...
    const contentType = contentTypeFinder(path);
    try {
...
function contentTypeFinder(requestPath) {
    ...
    const temp = extensionDictionary[extension];
    return extensionDictionary[extension] || 'application/octet-stream';
